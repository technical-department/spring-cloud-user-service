  ...
  <div>
    <el-form ref="form" :model="form" label-width="80px">
      <el-form-item label="系统">
        <el-select v-model="systemCode" placeholder="请选择" @change="selectChanged">
          <el-option
...
<script>
  import {getTree, editObj} from '@/api/sysrescource/sysrescource'
  import {queryList, checkPrecent, queryPage} from '@/api/sysbase/sysbase'
  ...
  export default {
    ...
    methods: {
      ...
      queryList() {
        ...
        queryList(this.systemCode).then(res => {
          this.baseinfos = res.data
          if (this.baseinfos.length > 0) {
            this.systemCode=this.baseinfos[0].systemCode
            this.systemName=this.baseinfos[0].systemName
            this.getTree(this.systemCode)
          }
      ...
      getTree() {
        this.listLoading = true
        getTree(this.systemCode).then(res => {
          this.list = res.data
          this.listLoading = false
