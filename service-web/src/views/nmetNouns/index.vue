...
<script>
import { tableOption } from '@/const/crud/nmet/nmetNouns'
import { queryList, updateStatus, delObject, addObj } from '@/api/nmet/nmetNouns'
export default {
  name: 'CommonProblem',
  data() {
    return {
  ...
  methods: {
    ...
    handleAdd(row, done, loading) {
      if (row.title === undefined || row.title === '') {
        this.$message.error('请填写名词')
        loading()
        return
      }
      if (row.content === undefined || row.content === '') {
        this.$message.error('请填写解释')
        loading()
        return
      }
      addObj(row.title, row.content, row.status).then(res => {
        this.getList(this.page)
        done()
        this.$notify({
          title: '成功',
