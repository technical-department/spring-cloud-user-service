<template>
  <div style="margin-top: 20px;">
    <avue-crud :option="option" :table-loading="listLoading" :data="list" :page="page" @search-change="searchChange" @on-load="getList" @refresh-change="searchChange">
...
<script>
import { tableOption } from '@/const/crud/university/recruitRecord'
import { queryList, delById, addObj, checkPrecent, deleteBatch } from '@/api/university/universityRecuitRecord'
export default {
  name: 'RecruitRecord',
  data() {
    return {
  ...
  methods: {
    searchChange(params) {
      ...
      this.getList({ currentPage: 1, pageSize: 20 }, this.tempParams)
    },
    getList(page, params) {
    ...
    confimAdd() {
      ...
      addObj(this.tempProviceId, this.tempIsArts, this.tempBatchNum, this.tempYear, this.uplodeUrl).then(res => {
    ...
    confimDelete() {
      ...
      deleteBatch(this.tempProviceId, this.tempIsArts, this.tempBatchNum, this.tempYear).then(res => {
