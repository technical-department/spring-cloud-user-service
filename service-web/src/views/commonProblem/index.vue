...
<script>
import { tableOption } from '../../const/crud/commonProblem/commonProblem'
import { queryList, updateStatus, delObject, addObj } from '@/api/commonProblem'
export default {
  name: 'CommonProblem',
  data() {
    return {
  ...
  methods: {
    ...
    handleAdd(row, done, loading) {
      addObj(row.question, row.answer, row.orderId, row.status).then(res => {
        this.getList(this.page)
        done()
        this.$notify({
          title: '成功',
